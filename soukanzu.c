/*目的: 2つのデータの相関図を作成するためのプログラム→gnuplotで図示*/
/*作成者: Ei Miura*/
/*最終更新日: 2020-09-22*/
...
void correlation_diagram(double *A, double *B, int n);//相関図作成
...
int main(int argc, char *argv[]){
  ...
  str2=(char*)e_calloc(num,sizeof(char),"str2");//メモリ確保
...
//相関係数を求める
void correlation_diagram(double *A, double *B, int n){
  int i;
  for(i=0;i<n;i++){
...
void output_comment(int n, int l, char **comment){
  ...
  fprintf(stdout, "# /////////////////////////////\n");
  fprintf(stdout, "# soukanzu.c (相関図を求めるプログラム)\n");
  fprintf(stdout, "# ./soukanzu < --.dat\n");
  fprintf(stdout, "# n= %d\n", n);
  today();
}
