/*目的: RPを求めるプログラム（出力を座標データではなく，行列とした） (Euclid距離のソートを行い，正確にプロット割合を定める) RP.c*/
/*作成者: Ei Miura*/
/*最終更新日: 2020-09-17*/
...
void RP2(double *L, int **RP2, int n, int m, double minl, double maxl, double theta_range);
//ユークリッド距離のソートで使用する
...
int main(int argc, char *argv[]){
  ...
  RP2(L,RP,n,m,minl,maxl,theta_range);
...
//RPの出力
void RP2(double *L, int **RP, int n, int m, double minl, double maxl, double theta_range){
  int i,p,q,l,l2;
  int plot_count=0;
  double plot_per=-1;
  l=0;
  ...
  plot_per=plot_count/(double)(n*(n-1));//n>5万だと計算できな
