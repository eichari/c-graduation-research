...
void standardize(long double *A, int n);//標準化を行う
long double average(long double *A, int n);//平均値を求める
long double variance(long double *A, int n);//分散を求める
...
//標準化を行う
void standardize(long double *A, int n){
  int i;
  long double ans=0;
  long double ave=average(A,n);
  long double var=variance(A,n);
  var=sqrt(var);
  for(i=0;i<n;i++){
    ans=(A[i]-ave)/var;
    fprintf(stdout, "%Lf \n",ans);
...
void output_comment(int n, int l, char **comment){
  ...
  fprintf(stdout, "# /////////////////////////////\n");
  fprintf(stdout, "# standardize.c (標準化を行うプログラム)\n");
  fprintf(stdout, "# ./standardize < --.dat\n");
  fprintf(stdout, "# n= %d\n", n);
  today();
}
