/*目的: 2次元データを1列目の時刻情報を使って昇順ソートするプログラム*/
/*作成者: Ei Miura*/
/*最終更新日: 2020-12-09*/
...
void lowsort(long double **A, int n, int m);//アトラクタを発火時刻順にソートする

void data_swap(long double **A, int m, int x, int y);//配列の x番目と y番目の要素を交換
void quicksort(long double **A, int p, int r, int m); /* 関数quicksort の宣言*/
int partition(long double **A, int p, int r, int m); /* 関数partition の宣言*/
...
//標準化を行う
void lowsort(long double **A, int n, int m){
  int i;
  quicksort(A,0,n-1,m);//昇順にソート
}

//2次元配列の x番目と y番目の要素を交換
void data_swap(long double **A, int m, int x, int y){
  int i;
  long double *TMP;

  TMP=(long double*)e_calloc(m,sizeof(long double),"TMP");
  //A[x][]とA[y][]を交換
  for(i=0;i<m;i++){TMP[i]=A[x][i];}
  for(i=0;i<m;i++){A[x][i]=A[y][i];}
  for(i=0;i<m;i++){A[y][i]=TMP[i];}
  free(TMP);
}
...
void output_comment(int argc, char *argv[], int n, int l, char **comment){
  ...
  fprintf(stdout, "# /////////////////////////////\n");
  fprintf(stdout, "# lowsort.c (2次元データを1列目の時刻情報で昇順ソートするプログラム)\n");
  fprintf(stdout, "# ./lowsort ");
  for(i=1;i<argc;i++){
