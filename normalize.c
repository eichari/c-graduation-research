...
void normalize(long double *A, int n);//正規化を行う
long double max(long double *A,int n);//埼大値を求める
long double min(long double *A,int n);//最小値を求める
...
//標準化を行う
void normalize(long double *A, int n){
  int i;
  long double minA=min(A,n);
  long double maxA=max(A,n);
  for(i=0;i<n;i++){
...
void output_comment(int n, int l, char **comment){
  ...
  fprintf(stdout, "# /////////////////////////////\n");
  fprintf(stdout, "# normalize.c (正規化を行うプログラム)\n");
  fprintf(stdout, "# ./normalize < --.dat\n");
  fprintf(stdout, "# n= %d\n", n);
  today();
}
